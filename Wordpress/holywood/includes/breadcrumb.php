...
if ( ! function_exists( 'ninetheme_holywood_breadcrubms' ) ) {
    function ninetheme_holywood_breadcrubms() {
        global $wp_query, $post, $paged;

        $space      = ' ';
        $on_front   = get_option( 'show_on_front' );
        $blog_page  = get_option( 'page_for_posts' );
        $separator  = $space . '<span class="bredcrumb-separator">' . apply_filters( 'ninetheme_holywood_breadcrumb_separator',
		'' ) . '</span>' . $space;
        $link       = apply_filters( 'ninetheme_holywood_breadcrumb_link', '<a href="%1$s" title="%2$s" rel="bookmark" class="breadcrumb-item">%2$s</a>' );
        $current    = apply_filters( 'ninetheme_holywood_breadcrumb_current', '<span class="breadcrumb-current">%s</span>' );
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'holywood' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'holywood' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'holywood' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category: ', 'holywood' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag: ', 'holywood' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'holywood' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'holywood' ) );
            }

        }

        $out .= '';
        echo apply_filters( 'ninetheme_holywood_breadcrumbs_out', $out );
    }
}
// Hooks into the Digital Store theme
add_action( 'ninetheme_holywood_before_template_header', 'ninetheme_holywood_breadcrubms' );